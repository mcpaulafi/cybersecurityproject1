...
def index(request):
    ...
    available_appointments = Appointment.objects.filter(start_date__gte=timezone.now(), user_id__isnull=True).order_by('start_date')

    # User's booked appointments
    user_appointments = Appointment.objects.filter(user_id=request.user.id).order_by('-start_date')
...
def booking(request):
    if request.method == 'POST':
        ...
        booked_time = Appointment.objects.get(id=request.POST.get('start_date_id'))
        # Check that date is not in the past
        if booked_time.start_date < timezone.now():
            messages.error(request, "You cannot book a time in the past.", extra_tags="booking")
            return redirect('index')

        note = request.POST.get('note')

# SECURITY FLAW 3: Injection
# Fix by removing comments from the 3 lines below
...
def question(request):
    if request.method == 'POST':
        ...
        if not re.match(r'^[\w\s.,!?-]*$', answer_text):
            messages.error(request, "Answer contains invalid characters!", extra_tags="answer_check")
            return redirect('question')
        if len(answer_text) < 4:
            messages.error(request, "Answer must be at least 4 characters long.", extra_tags="answer_check")
            return redirect('question')
        ...
        if (question_id is not None) and (answer_text is not None):
            ...
            # Add success message
            messages.success(request, "Recovery question saved successfully!", extra_tags="answer")
            return redirect('index')
...
def forgot(request):
    if request.method == 'POST':
        ...
        if not re.match(r'^[\w\s.,!?-]*$', username):
            messages.error(request, "Username contains invalid characters!", extra_tags="answer_check")
            return redirect('forgot')
        ...
        try:
            ...
            messages.error(request, "Check username!", extra_tags="answer_check")
            return redirect('forgot')
        except Question.DoesNotExist:
            messages.error(request, "Check question!", extra_tags="answer_check")
            return redirect('forgot')
        ...
        if exists:
            # Add success message
            messages.success(request, "Recovery question was answered correctly!", extra_tags="answer_check")
            context = {
            "user" : user
            }
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if pswd1 != pswd2:
                ...
                return render(request, "pages/changepswd.html", context)
            if username == pswd1:
                messages.error(request, "Username and password should not match!", extra_tags="pswd_check")
                return render(request, "pages/changepswd.html", context)
            if len(pswd1) < 8:
                messages.error(request, "Password must be at least 8 characters long.", extra_tags="pswd_check")
                return render(request, "pages/changepswd.html", context)
            # if not re.search(r'\d', pswd1):
            #     messages.error(request, "Password must contain at least one number.", extra_tags="pswd_check")
            #     return redirect('changepswd')
            # if not re.search(r'[A-Z]', pswd1):
            #     messages.error(request, "Password must contain at least one uppercase letter.", extra_tags="pswd_check")
            #     return redirect('changepswd')
            # if not re.search(r'[a-z]', pswd1):
            #     messages.error(request, "Password must contain at least one lowercase letter.", extra_tags="pswd_check")
            #     return redirect('changepswd')
            # if not re.search(r'[!@#$%^&*(),.?":{}|<>]', pswd1):
            #     messages.error(request, "Password must contain at least one special character.", extra_tags="pswd_check")
            #     return redirect('changepswd')
